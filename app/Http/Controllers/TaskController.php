...
class TaskController extends Controller
{
  ...
  public function search(Request $request)
  {
    ...
    $domain = Domain::query()->where('domain', $request['domain'])->first();
    $country = Company::query()->where('country', $request['country'])->first();

    if (isset($domain)) {
      if (isset($country)) {
        $post = Post::query()->where('domain_id', $domain['id'])
        ->where('location', $country['country'])->latest()->get();
      } else {
    ...
    if (isset($country)) {
    $post = Post::query()->where('location', $country['country'])->latest()->get();
    }
    if (isset($post))
      return response()->json([
        $post
